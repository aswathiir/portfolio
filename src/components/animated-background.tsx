...
const AnimatedBackground = () => {
  ...
  const applyIconsToKeycaps = async () => {
    if (!splineApp) return;
    
    console.log("🎨 Starting to apply icons to keycaps...");
    
    for (const skill of Object.values(SKILLS)) {
      try {
        // Find the keycap object by skill name
        const keycap = splineApp.findObjectByName(skill.name);
        if (!keycap) {
          console.warn(`⚠️ Keycap not found for skill: ${skill.name}`);
          continue;
        }
        ...
        await new Promise((resolve, reject) => {
          img.onload = () => {
            try {
              // Create a canvas to convert image to texture
              ...
              if (ctx) {
                ...
                // Apply to the keycap's material
                if (keycap.material) {
                  // Try to set the texture as the material's map
                  splineApp.setTexture(keycap, textureUrl);
                  console.log(`✅ Applied icon for ${skill.label}`);
                } else {
                  console.warn(`⚠️ No material found for ${skill.name}`);
                }
              }
              resolve(true);
            } catch (error) {
              console.error(`❌ Error processing icon for ${skill.name}:`, error);
              reject(error);
            }
          };
        ...
        // Small delay between icon applications to avoid overwhelming the browser
        await sleep(100);
        
      } catch (error) {
        console.error(`❌ Error applying icon for ${skill.name}:`, error);
      }
    }
    
    console.log("🎉 Finished applying all icons!");
  };
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <Spline
        ...
        scene="/assets/skills_keyboard_updated.spline"
      />
    </Suspense>
  );
};
